...
(async()=>{
 ...
 try{
  ...
  // The current build prepares the initial view asynchronously.  Capturing before
  ...
  for(const [id,title] of shots.filter(s=>!selected.size||selected.has(s[0]))){
   const r=await page.evaluate(async({id,title,base})=>{
    ...
    let s;
     ...
     const destination=id==='ultra-desert'?'biome-2':id==='polar-station'?'port-0':id==='shade-structure'?'shade-0':id==='mycelium'?'biome-8':id==='cloudscape'?'biome-1':'exterior-1';
     SphereEvolution.visit(destination,{walk:id==='mycelium'});s=A.getState();
     if(id==='ultra-desert'){
      // A focused hero-landscape photograph: lock the matching biome rather than
      // allowing the wide-area atlas beneath this address to choose a neighbour.
      const f=SphereSites.shellFrame(s.siteAnchor);s.biome=2;s.geometryDetail=false;s.routeShades=false;s.multipleWounds=false;s.starStation=false;s.shineField=false;
      s.position=M.mul(s.siteAnchor,s.radius-1);s.forward=M.norm(M.add(f[2],M.mul(f[1],-.38)));s.up=M.basis(s.forward,f[1]).u;s.fov=76;s.atmosphere=.72;s.weatherStrength=.22;s.exposure=.05;
     }
  ...
  assert.deepEqual(errors,[]);fs.writeFileSync(path.join(dir,'gallery.json'),JSON.stringify({version:'current-build',method:'Unretouched Observatory renders at 1920 × 1080, internally supersampled at 3840 × 2160',shots:results},null,2)+'\n');
