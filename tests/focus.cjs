...
const state={...s,geometryDetail:true,siteId:'biome-2',siteAnchor:W.locateBiome(2,s)},mesh=S.site(state);state.position=mesh.world([0,.15,0]);const landed=F.move(state,M.mul(mesh.basis[1],-1),10);assert(landed.blocked);const end=mesh.local(landed.position),contact=F.contact(state,state.position,M.mul(mesh.basis[1],-1),10);near(mesh.local(M.add(state.position,M.mul(mesh.basis[1],-contact.distance)))[1],S.ground(mesh,0,0)+F.CLEARANCE,.000002);assert(end[1]>=S.ground(mesh,end[0],end[2])+F.CLEARANCE-.000002);
