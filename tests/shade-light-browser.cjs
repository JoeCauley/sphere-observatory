...
for(const samples of [7,19])for(const shape of ['disk','square','cap','trimmed'])for(const hour of [0,8,12]){
 const s={...M.defaultState(),collection:true,era:'before',shadeShape:shape,time:hour*3600,shadowSamples:samples,starStation:false,viewMode:'lightid',fov:100};const pl=C.plates(s).find(p=>p.id===16);
