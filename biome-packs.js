...
(function(root){
'use strict';
const watershed=Object.freeze({id:'watershed',geographyRevision:1,artRevision:2,
 ...
 ...[['lake','Receiving lake','Silver-green shores around an open receiving basin.'],['reach','Quiet reach','A long wooded waterway connecting the garden catchments to the lake.'],['meadow','Open meadow','Golden-green open country beside the quiet reach.']].map(([composition,name,description])=>({id:'watershed-'+composition,packId:'watershed',composition,name:'Watershed · '+name,category:'watersheds',description,biome:0,walk:true}))]
});
function validate(address,s){
 if(address==null)return null; // Migration: historical scenes never acquire a new atlas.
 if(typeof address!=='object'||Array.isArray(address)||address.schema!==1||address.packId!==watershed.id||address.geographyRevision!==1||![1,2].includes(address.artRevision))throw Error('Unsupported Biome Pack address or revision');
 ...
 return {schema:1,packId:watershed.id,geographyRevision:1,artRevision:address.artRevision,seed:address.seed,anchor:address.anchor.slice()};
}
function activate(input){const s=root.SphereWatershed.activate(input);s.packAddress={schema:1,packId:watershed.id,geographyRevision:1,artRevision:input.packAddress?.artRevision??watershed.artRevision,seed:s.provinceSeed,anchor:s.provinceAnchor.slice()};const clean=root.SphereMath.validate(s);clean.playing=!!input.playing;return clean;}
