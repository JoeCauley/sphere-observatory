...
function model(seed=713){
 ...
 const result={seed,edges,nodes,junctions,catchments:catchments.map(([x,z,r],i)=>({x,z,r,id:i})),lake:{x:lake.x,z:lake.z,rx:2100,rz:1100},core};cache.set(seed,result);while(cache.size>2)cache.delete(cache.keys().next().value);return result;
}
...
function sample(q,s){if(!root.SpherePacks.enabled(s))return null;const [x,z]=P.local(q,s);if(!Number.isFinite(x))return null;const v=sampleLocal(x,z,s.provinceSeed);return v?{...v,x,z,revision:1,seed:s.provinceSeed}:null;}
root.SphereNeighbourhood={model,mask,sampleLocal,sample,get info(){return {addresses:cache.size,segments:[...cache.values()].reduce((n,p)=>n+p.edges.length,0),graphBytes:[...cache.values()].reduce((n,p)=>n+p.edges.length*272+p.nodes.length*48,0),textureBytes:0};}};
