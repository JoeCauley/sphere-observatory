...
const panel=document.createElement('section');panel.id='flightConsole';panel.hidden=true;panel.setAttribute('aria-label','Flight console');panel.innerHTML=`
 ...
 <label>Speed control<select id="consoleSpeedMode"><option value="auto">Automatic</option><option value="manual">Manual</option></select></label>
 <div class="console-grid"><label>Manual flight speed<select id="consoleSpeed" aria-label="Flight console flight speed"></select></label><label>Lens <output id="consoleFovValue"></output><input id="consoleFov" type="range" min="25" max="120" step="1" aria-label="Flight console field of view"></label></div>
 ...
 <p class="micro">Drag to look · W A S D fly · Q / E descend and climb · Space + E lift off · Space play / pause · H hide controls</p>`;
...
function update(key,value){const s=A.getState();s[key]=value;if(key==='speed')s.autoSpeed=false;A.setState(s);}
$('consolePlay').onclick=()=>{A.setPlaying(!A.getState().playing);focus();};
$('consoleReset').onclick=()=>{const s=A.getState();s.time=0;A.setState(s);focus();};
$('consoleScrub').oninput=e=>{const s=A.getState();s.time=Number(e.target.value);A.setState(s);};
...
$('consoleSpeedMode').onchange=e=>{update('autoSpeed',e.target.value==='auto');focus();};
...
function sync(){const s=A.getState(),ground=SphereInspection.surface(s);$('consolePlay').textContent=s.playing?'Pause':'Play';$('consolePlay').setAttribute('aria-label',s.playing?'Pause simulation':'Play simulation');$('consoleTime').textContent=$('timeReadout').textContent;
 ...
 $('consoleSpeedMode').value=s.autoSpeed===false?'manual':'auto';$('consoleSpeed').disabled=s.autoSpeed!==false;$('consoleLevel').checked=s.surfaceLock;$('consoleMaterials').value=s.richMaterials?'1':'0';$('consoleShadows').value=String(s.localShadows);$('consoleAntialias').value=String(s.antialias);
