...
function terrain(mesh,id,s){const step=SITE_RADIUS*2/GRID,heights=new Float64Array((GRID+1)**2),province=root.SphereWatershed;
 ...
 for(let z=0;z<GRID;z++)for(let x=0;x<GRID;x++){const a=p(x,z),b=p(x+1,z),c=p(x+1,z+1),d=p(x,z+1);
  ...
  mesh.tri(a,c,b,[1,1,1],id);mesh.tri(a,d,c,[1,1,1],id);}
}
...
function populate(mesh,id,s){const rnd=random(s.seed*8191+id*977),colour=W.palette[id];
 ...
 for(let i=0;i<320;i++){const x=(rnd()*2-1)*1.08,z=(rnd()*2-1)*1.08;if(Math.hypot(x,z)<.055||protectedPlace(x,z))continue;const y=ground(mesh,x,z),r=.003+rnd()*.009,h=.008+rnd()*.036,a=rnd()*Math.PI*2,variation=.6+rnd()*.6,c=colour.map(v=>v*variation);
...
const cache=new Map();
function site(s){if(!s.siteId||!s.siteAnchor||s.layoutVersion!==2)return null;const key=JSON.stringify([s.siteId,s.siteAnchor,s.siteRevision,s.siteElevation,s.provinceRevision,s.provinceSeed,s.provinceAnchor,s.radius,s.seed,s.era,s.shellThickness]);if(cache.has(key))return cache.get(key);let mesh;
...
function enterWalk(s){const mesh=site(s);if(!mesh?.ground)throw Error('Choose a biome or polar entry field site first.');s.walkMode=true;s.walkSurface=false;s.walkPosition=[0,ground(mesh,0,0)+EYE+.0005,0];s.position=mesh.world(s.walkPosition);s.forward=norm(add(mesh.basis[2],mul(mesh.basis[1],.035)));s.up=M.basis(s.forward,mesh.basis[1]).u;s.fov=82;s.projection='perspective';s.walkVelocity=0;jumpHeld=false;return s;}
...
root.SphereSites={Mesh,site,rim,shadeSection,exterior,geometry,ground,blocked,height,noise,step,enterWalk,rayTriangle,rayBVH,packBVH,trace,shellFrame,SITE_RADIUS,EYE};
