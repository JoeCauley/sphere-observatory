...
function previewBiome(){const b=B.catalog[Number($('biomeDestination').value)];$('biomePreviewImage').setAttribute('aria-label',b.name+' aerial texture preview');$('biomePreviewCaption').textContent=b.description;}
...
function navigate(name){if(name==='surface'){SphereSurface.visit();journeys.querySelector('[data-destination="surface"]').classList.add('selected');return;}SphereJourney.remember();SphereSurface.resetReturn();const s=A.getState();s.walkMode=false;s.walkSurface=false;s.walkPosition=null;s.siteId='';s.siteAnchor=null;s.shadeAttachment=null;A.setState(s);
