...
function view(name,{stage=false}={}){const prior=A.getState();change(s=>{s.collection=true;s.atmosphere=1;s.exposure=.2;s.shellshine=.22;s.fov=80;s.speed=1000000;s.projection='perspective';s.viewMode='material';
...
const light=$('lighting');const aaNote=document.createElement('p');aaNote.className='micro';aaNote.textContent='Smooth edges adds extra samples along Shade outlines and cracks, even when the preview has reached its pixel budget. It also filters remaining contrast edges and can soften fine structure. Supersampling only uses the unfiltered light resolve.';$('antialias').closest('label').after(aaNote);
