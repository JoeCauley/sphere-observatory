/* Revision 2: automatic boundaries of the existing combined Shade footprint.
 * A 180 m local edge section hangs beneath the unchanged thin skin.
 * No raised deck or new footprint is implied. Revision 1 keeps its service strip.
 */
(function(root){
...
function curves(p){
 ...
 else if(p.shape==='trimmed'){const angle=Math.acos(p.trim),z=Math.sqrt(1-p.trim*p.trim);curve('north-arc',angle,Math.PI-angle,t=>[Math.cos(t),Math.sin(t)],'intact');curve('south-arc',Math.PI+angle,2*Math.PI-angle,t=>[Math.cos(t),Math.sin(t)],'intact');line('east-trim',[p.trim,-z],[p.trim,z]);line('west-trim',[-p.trim,z],[-p.trim,-z]);}
 else curve('perimeter',0,2*Math.PI,t=>[Math.cos(t),Math.sin(t)],'intact');
 if(p.damage){
...
function nearest(curve,uv){let best=Infinity,t=curve.lo;const distance=t=>{const q=curve.at(t);return (q[0]-uv[0])**2+(q[1]-uv[1])**2;},step=(curve.hi-curve.lo)/64;
...
function candidates(s,range){if(!s.collection||s.layoutVersion!==2)return [];const result=[];
 for(const p of C.plates(s)){const r=len(p.center)*s.radius,curved=p.shape==='cap'||p.shape==='trimmed',plane=curved?Math.abs(len(s.position)-r):Math.abs(dot(s.position,p.normal)-r);if(plane>range+.2||dot(s.position,p.normal)<=0)continue;
  ...
  for(const c of curves(p)){const t=nearest(c,uv),point=E.plateVector(E.canonicalPoint(s,p,...c.at(t)),p),distance=len(sub(s.position,point));if(distance<range+2)result.push({p,c,t,distance});}
 }return result.sort((a,b)=>a.distance-b.distance);
}
function meshes(s,options={}){const out=[],range=E.readableRange(.18,s)+Math.min(E.readableRange(.18,s),s._edgePrepareKm||0),fp=E.focal(s);
 for(const {p,c,t}of candidates(s,range*1.2)){const scale=p.size*s.radius*Math.max(1,p.across||1),step=3.2/scale,centre=Math.floor((t-c.lo)/step),count=Math.ceil(range*1.2/3.2)+2;
  for(let j=Math.max(0,centre-count);j<=centre+count;j++){const a=c.lo+j*step,b=Math.min(c.hi,a+step);if(a>=c.hi)break;
   ...
   const fine=!options.coarse&&(options.fine||distance<.025*fp),lod=fine?'near':'far',key='shade:'+JSON.stringify([2,p.id,s.radius,p.shape,p.across,p.damage,p.trim,c.id])+':'+j+':'+lod;
   const build=()=>{const f={origin:point,basis:[[1,0,0],[0,1,0],[0,0,1]]},mesh=new S.Mesh(point,f.basis,c.kind==='intact'?'Shade · finished perimeter':'Shade · broken structure');mesh.canonicalOrigin=point;mesh.canonicalBasis=f.basis;mesh.parentId=p.id;
    ...
    mesh.tri=function(a,b,c,colour,material=-1,emission=0){const input=[a,b,c],inside=input.map(contains);if(inside.every(Boolean)){raw.call(this,a,b,c,colour,material,emission);return;}if(inside.every(v=>!v))return;const polygon=[];
    ...
    const vertex=(fr,x,y)=>sub(add(add(fr.origin,mul(fr.inward,x)),mul(fr.up,y)),point),n=fine?16:4;
    ...
    for(let i=0;i<n;i++){let lo=a+(b-a)*i/n,hi=a+(b-a)*(i+1)/n,fa=frame(s,p,c,lo+1e-14),fb=frame(s,p,c,hi-1e-14),fm=frame(s,p,c,(lo+hi)/2);if(!fa&&!fb&&!fm)continue;
     ...
     const strata=c.kind==='intact'?[0,-.18]:[0,-.025,-.075,-.145,-.18];
     const offset=k=>c.kind==='intact'?0:[0,0,.018,.01,0][k];
     for(let k=1;k<strata.length;k++){if(c.kind==='broken'&&(k===2||k===3))continue;const colour=c.kind==='intact'?[.12,.14,.15]:[.065+k*.008,.073+k*.008,.078+k*.008];mesh.quad(vertex(fa,offset(k-1),strata[k-1]),vertex(fb,offset(k-1),strata[k-1]),vertex(fb,offset(k),strata[k]),vertex(fa,offset(k),strata[k]),colour,-4);}
     if(c.kind==='broken'){mesh.quad(vertex(fa,.045,-.025),vertex(fb,.045,-.025),vertex(fb,.045,-.145),vertex(fa,.045,-.145),[.035,.042,.048],-4);if(fine){mesh.beam(vertex(fa,.012,-.026),vertex(fb,.012,-.145),.008,[.15,.13,.095]);mesh.beam(vertex(fa,.012,-.145),vertex(fb,.012,-.026),.008,[.12,.14,.15]);}}
    }
    delete mesh.tri;mesh.shadeRange=[a,b];mesh.boundaryId=c.id;mesh.lod=lod;mesh.sectionRevision=2;return mesh.finish({deferBVH:true});};
   const mesh=E.remember(key,build,distance,distance<1);if(mesh&&mesh.count)out.push(E.transformMesh(mesh,p));
  }
 }return out;
}
root.SphereShadeEdges={curves,solid,frame,nearest,uvAt,candidates,meshes};
