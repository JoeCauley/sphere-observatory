...
function contact(s,p,d,distance,skipShell=false){let hit={distance:Infinity,normal:null,kind:''};
 ...
 if(s.collection)for(const plate of C.plates(s)){const t=C.diskDistance(M.mul(p,1/s.radius),d,plate)*s.radius;if(t<=distance+CLEARANCE*100){const n=plate.shape==='cap'||plate.shape==='trimmed'?M.norm(M.add(p,M.mul(d,t))):plate.normal,safe=Math.max(0,t-CLEARANCE/Math.max(.01,Math.abs(M.dot(n,d))));if(safe<=distance&&safe<hit.distance)hit={distance:safe,normal:M.mul(n,M.dot(n,d)>0?-1:1),kind:'Shade'};}}
 else if(s.shadeEnabled){const plate=M.shade(s),t=M.plateDistance(p,d,plate),safe=Math.max(0,t-CLEARANCE/Math.max(.01,Math.abs(d[2])));if(safe<=distance&&safe<hit.distance)hit={distance:safe,normal:[0,0,d[2]>0?-1:1],kind:'Shade'};}
 if(s.geometryDetail&&root.SphereSites){const S=root.SphereSites,meshes=new Set([...S.geometry({...s,position:p}),...(root.SphereEdges?.shadeContactMeshes(s,p,d,distance)||[])]);for(const mesh of meshes){const record={},local=mesh.local(p),dir=mesh.basis.map(b=>M.dot(d,b)),t=S.rayBVH(local,dir,mesh.bvh,Math.min(distance+1,hit.distance+1),record);if(!record.normal)continue;let n=mesh.basis.reduce((v,b,i)=>M.add(v,M.mul(b,record.normal[i])),[0,0,0]);if(M.dot(n,d)>0)n=M.mul(n,-1);const safe=Math.max(0,t-CLEARANCE/Math.max(.01,-M.dot(n,d)));if(safe<hit.distance&&safe<=distance)hit={distance:safe,normal:n,kind:mesh.name};}}
 return hit;
}
function move(s,d,distance){const p=s.position;if(!(distance>0))return {position:p.slice(),blocked:false};d=M.norm(d);
 ...
 let position=p.slice(),blocked=false,kind='',curvedSlide=false,firstContact=null;
 for(let iteration=0;iteration<3&&distance>.0000001;iteration++){
  ...
  if(iteration===0)firstContact=hit.distance;
 ...
 return {position,blocked,kind,firstContact};
}
