(function(){
 ...
 let measuring=false,points=[],drawing=false,result=null,signature='',busy=false,cancelled=false,resultText='';
 function pose(s){return JSON.stringify([s.position.map(v=>Math.round(v*1e4)),s.forward.map(v=>Math.round(v*1e10)),s.up.map(v=>Math.round(v*1e10)),s.radius,s.fov,s.projection,vp.clientWidth/vp.clientHeight]);}
 function clear(){points=[];result=null;signature='';resultText='';$('surveyPath').setAttribute('d','');$('surveyPath').style.visibility='visible';$('exportSurvey').disabled=true;$('surveyResult').textContent='Drag over the surface. Shades, star and open space are excluded.';}
 ...
 function showMeasure(active){measuring=active;$('surveyControls').hidden=!measuring;$('measureButton').classList.toggle('active',measuring);$('measureButton').setAttribute('aria-pressed',String(measuring));canvas.classList.toggle('measuring',measuring);$('surveyOverlay').style.visibility=measuring?'visible':'hidden';window.dispatchEvent(new Event('sphere-measure-mode'));}
 function toggleMeasure(){if(busy)return;showMeasure(!measuring);if(measuring){if(!result)clear();A.toast('Area tool on. Draw a rectangle or trace a freehand region. The result stays until you clear it.');}}
 ...
 canvas.addEventListener('pointerup',async e=>{if(!measuring||!drawing)return;e.stopImmediatePropagation();drawing=false;busy=true;$('surveyResult').textContent='Measuring the curved surface…';
  try{await new Promise(r=>setTimeout(r,20));const s=A.getState(),r=vp.getBoundingClientRect();result=SphereSurvey.measure(s,r.width/r.height,points);signature=pose(s);const f=new Intl.NumberFormat(undefined,{maximumSignificantDigits:5});let earths=result.earths>=1e6?f.format(result.earths/1e6)+' million':f.format(result.earths);
   const excluded=result.gross?Math.round((1-result.area/result.gross)*100):0;resultText=`${earths} Earth surfaces · ${f.format(result.area)} km². ${excluded}% hidden or open space. Sampling difference: ${(result.relativeDifference*100).toFixed(2)}%. Measured at simulation second ${s.time.toFixed(1)}; the saved result keeps that scene.`;$('surveyResult').textContent=resultText;$('exportSurvey').disabled=false;
  }catch(err){A.toast('Could not measure: '+err.message);}finally{busy=false;A.setBusy(false);}
 },true);
 canvas.addEventListener('pointercancel',()=>{if(!drawing)return;drawing=false;A.setBusy(false);clear();});
 ...
 function checkMeasurement(s=A.getState()){if(!drawing&&result){const same=pose(s)===signature;$('surveyPath').style.visibility=same?'visible':'hidden';$('surveyResult').textContent=resultText+(same?'':' This measurement belongs to the previous view.');}}
 window.addEventListener('sphere-view-changed',()=>checkMeasurement());
 ...
 setInterval(()=>{if(document.hidden||busy||drawing)return;const s=A.getState(),hit=M.trace(s.position,s.forward,s),alt=s.radius-M.length(s.position);checkMeasurement(s);
 ...
 function shot(name,start,t){const s=structuredClone(start);
 ...
 async function recordClip(options={}){
  ...
  try{
   await A.renderer.prepare(start,{aspect:width/height});A.renderer.draw(shot(name,start,0),width,height,{exportFrame:true});stream=canvas.captureStream(0);const track=stream.getVideoTracks()[0];if(!track.requestFrame)throw Error('Manual video capture is unavailable in this browser.');
